...
.venv/bin/python -m knowbase import <文档目录> --type standard             # 标准类强制 staging
```

导入后建议让 AI 逐条 `memory_update` 提炼 staging 内容。standard/preference/bizrule 激活后，Agent 不可再直接修改；人工使用 `knowbase revise`。
...
- 默认 MCP 检索仅 active 且非 staging；无项目上下文只查 global。有项目上下文时必须通过 `scope_map`、已存在的同名 scope 或显式参数确定项目，无法解析就拒绝检索，避免跨项目召回。自动 Hook 与 MCP 使用同一混合检索路径。
